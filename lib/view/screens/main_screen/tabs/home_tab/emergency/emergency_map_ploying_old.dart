...
//       message: "Cannot detect your location. Please enable GPS and try again.",
//     );
//     return;
//   }
//
...
//     dev.log("📍 Current Position: lat=${currentPosition?.latitude}, long=${currentPosition?.longitude}");
//
//     // Fetch user cars
//     final cars = await CarApi.getCars(locale: context.locale);
//     dev.log("🚗 Found ${cars.length} cars for user");
//
//     if (cars.isEmpty) {
//       showSnackbar(
//         context: context,
//         status: SnackbarStatus.error,
//         message: "No cars found. Please add a car first.",
//       );
//       return;
//     }
//
//     // Get default car, or first car as fallback
//     final defaultCar = cars.firstWhereOrNull((element) => element.isDefault ?? false);
//     final firstCar = cars.isNotEmpty ? cars.first : null;
//     final selectedCar = defaultCar ?? firstCar;
//
//     // Check if we have a valid car
//     if (selectedCar == null) {
//       showSnackbar(
//         context: context,
//         status: SnackbarStatus.error,
//         message: "No valid car found. Please add a car first.",
//       );
//       return;
//     }
//
//     final userCarId = selectedCar.id;
//     final userCar = '${selectedCar.car?.model?.brand?.name ?? "Unknown"} - ${selectedCar.car?.model?.name ?? "Model"} - ${selectedCar.car?.year ?? ""}';
//
//     dev.log("🚗 Selected car ID: $userCarId, Car: $userCar");
//
//     try {
...
//         }
//         dev.log("📍 Address resolved: $address");
//       } else {
//         // Fallback address is just coordinates
//         address = "${currentPosition!.latitude}, ${currentPosition!.longitude}";
//         dev.log("⚠️ Could not resolve address, using coordinates");
//       }
//       }
//     }
//   }
...
//     // Check if already searching to prevent multiple requests
//     if (_isSearching) {
//       return;
//     }
//
//     // Validate description is not empty
//     if (_orderDetailsController.text.isEmpty) {
//       showSnackbar(
//         context: context,
//         status: SnackbarStatus.error,
//         message: "Please enter details about your emergency",
//       );
//       return;
//     }
//
...
//         );
//       } catch (e) {
//         dev.log("⚠️ Failed to get address from coordinates", error: e);
...
//             '${place.locality ?? ""}, ${place.postalCode ?? ""}, '
//             '${place.country ?? ""}';
//         address = address.replaceAll(RegExp(r', ,'), ",").replaceAll(RegExp(r',{2,}'), ",").trim();
//         if (address.endsWith(", ")) {
//           address = address.substring(0, address.length - 2);
//         }
//         dev.log("📍 Address resolved: $address");
...
//       }
//
//       dev.log("🚑 Creating emergency order with car ID: $userCarId");
...
//         dev.log("✅ Emergency order created successfully with ID: $orderId");
...
//         });
//       } else {
//         throw Exception("Created order has no ID");
//       }
//     } catch (e) {
//       dev.log("❌ Emergency order creation failed", error: e);
//       showSnackbar(
//         context: context,
//         status: SnackbarStatus.error,
//         message: "Failed to create emergency request: ${e.toString()}",
//       );
//     } finally {
//       if (mounted) {
//         setState(() {
