...
class _ProductDetailsScreenState extends State<ProductDetailsScreen> {
      ...
      body: Padding(
        ...
        child: ListView(
          ...
          children: [
            ...
            // Only show image thumbnails if there are multiple images
            if (_hasMultipleImages())
              Column(
                children: [
                  UtilValues.gap8,
                  _buildImageThumbnails(widget.product.images ?? []),
                ],
              ),
            UtilValues.gap16,
            ...
            Row(
              ...
              children: [
                if (productProvider.tires.contains(widget.product))
                  Container(
                    width: 200,
                    decoration: BoxDecoration(
                    ...
                    child: Row(
                      ...
                      children: [
                        Padding(
                          ...
                          child: InkWell(
                            onTap: () => setState(() => productProvider
                                .addIncrementTires(widget.product)),
                            child: SizedBox(
                              height: 15,
                              width: 15,
                        ...
                        IconButton(
                          onPressed: () => setState(() =>
                              productProvider.decrementTires(widget.product)),
                  ...
                  SizedBox(
                    ...
                    child: SimplePrimaryButton(
                      ...
                      onPressed: () => setState(() =>
                          productProvider.addIncrementTires(widget.product)),
                    ),
                  ),
                SizedBox(
                  height: 100,
  ...
  // Check if the product has multiple images to display
  bool _hasMultipleImages() {
    return widget.product.images != null && widget.product.images!.length > 1;
  }
  
  // Main image display - either a carousel or single image based on available images
  Widget _buildMainImageDisplay() {
    // If there are multiple images and we should display a carousel
    if (_hasMultipleImages()) {
      return Container(
        height: 30.h, // Use 30% of screen height as in vendor app
        margin: EdgeInsets.symmetric(horizontal: 4.sp), // Add horizontal margins
        child: Stack(
          children: [
            CarouselSlider.builder(
              ...
              options: CarouselOptions(
                ...
                onPageChanged: (index, reason) {
                  setState(() {
                    // Update current index if needed for indicators
                  });
                },
              ),
            ...
            Positioned(
              ...
              child: Row(
                ...
                children: widget.product.images!.asMap().entries.map((entry) {
                  return Container(
                    width: 8.0, // Match vendor app size
                    height: 8.0, // Match vendor app size
                    margin: EdgeInsets.symmetric(horizontal: 4.0),
                    decoration: BoxDecoration(
                      shape: BoxShape.circle,
                      color: Colors.white.withOpacity(
                        // Can use a state variable to track current index
                        // For now, make first dot always highlighted
                        entry.key == 0 ? 0.9 : 0.4,
                      ),
                    ),
                  );
    ...
    // Display a single image (either the first image or the thumbnail)
    else {
      String imageUrl = '';
      
      // First try to use an image from the images array if available
  ...
  // Horizontal scrolling thumbnails for multiple images
  Widget _buildImageThumbnails(List<Thumbnail> sliders) {
